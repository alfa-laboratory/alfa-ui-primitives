import React from "react";

const SvgIconBankYandexmoneyLColor = props => (
  <svg width={30} height={30} {...props}>
    <defs>
      <path
        id="icon_bank-yandexmoney_l_color_svg__a"
        d="M.006.087h19.949V27.5H.005z"
      />
    </defs>
    <g fill="none" fillRule="evenodd">
      <path d="M0 0h30v30H0z" />
      <g transform="translate(5 1.25)">
        <mask id="icon_bank-yandexmoney_l_color_svg__b" fill="#fff">
          <use xlinkHref="#icon_bank-yandexmoney_l_color_svg__a" />
        </mask>
        <path
          d="M0 13.685c0-1.54.124-2.516 3.175-4.896C5.7 6.82 13.798.087 13.798.087v11.238h6.157V27.5H1.923C.866 27.5 0 26.595 0 25.49V13.685"
          fill="#FACA22"
          mask="url(#icon_bank-yandexmoney_l_color_svg__b)"
        />
        <path
          fill="#C9A521"
          mask="url(#icon_bank-yandexmoney_l_color_svg__b)"
          d="M13.798 11.325v6.512L2.483 25.89l14.305-4.863v-9.702h-2.99"
        />
        <path
          d="M8.627 10.988c.662-.823 1.628-1.114 2.159-.649.53.465.424 1.51-.238 2.334-.66.824-1.627 1.114-2.158.649-.53-.465-.424-1.51.237-2.334"
          fill="#6F8F9D"
          mask="url(#icon_bank-yandexmoney_l_color_svg__b)"
        />
      </g>
    </g>
  </svg>
);

export default SvgIconBankYandexmoneyLColor;
